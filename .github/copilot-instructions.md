...
- **Package name**: `@kids-reading-tracker/api-types`
- **Entry point**: `packages/api-types/src/index.ts` — exports friendly type aliases and
  runtime constants (`SHELF_LABELS`, `SHELF_ORDER`) re-exported from the generated schema.
- **Generated file**: `packages/api-types/src/generated.ts` — auto-generated by
  `openapi-typescript` from the live API schema. Never edit this file by hand.
- **Regenerate**: `npm run generate:types` from the repo root (requires the Django API
  to be running at `http://localhost:8000`).
- **Consumers**: Both `apps/web` and `apps/mobile` declare `"@kids-reading-tracker/api-types": "*"`
  in their `package.json` dependencies and resolve the package through npm workspaces.

#### Type-generation rules

1. **Never define inline interfaces that duplicate API types.** Any type that mirrors an
   API model (e.g. `User`, `ShelfEntry`, `BookBrief`) must be imported from
   `@kids-reading-tracker/api-types`, not declared locally.

2. **Keep the schema up to date.** Whenever you add or modify a Django model, serializer,
   or view that changes the API contract, regenerate the types:
   ```bash
...
3. **Annotate plain `APIView` subclasses** with `@extend_schema` from `drf_spectacular`
   so the generated schema contains correct request/response shapes. Plain `APIView`
   classes are not introspected automatically.

4. **Update `packages/api-types/src/index.ts`** if you add a new schema that consumers
   will need. Add a type alias under the appropriate section (auth, library, etc.).
